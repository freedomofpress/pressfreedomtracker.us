# Generated by Django 3.2.19 on 2023-05-25 22:01
...
def update_categories_in_incident_set(apps, schema_editor):
    ...
    for page in BlogPage.objects.all():
        ...
        for block in page.body.raw_data:
            if block['type'] in ('vertical_bar_chart', 'tree_map_chart', 'bubble_map_chart'):
                incident_set = block['value']['incident_set']
                category_titles = []

                for category in incident_set.get('categories', []):
                    if isinstance(category, str):
                        category_titles.append(category)
                    elif pk := category.get('value', None):
                        try:
                            title = CategoryPage.objects.get(pk=pk).title
                            category_titles.append(title)
                            should_update = True
                        except CategoryPage.DoesNotExist:
                            pass
                incident_set['categories'] = category_titles
        ...
        if should_update:
            ...
            page.body = wagtail.blocks.StreamValue(
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RunPython(update_categories_in_incident_set, reverse_code=migrations.RunPython.noop, elidable=True),
    ]
