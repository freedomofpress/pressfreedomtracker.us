...
class CleanTest(TestCase):
    def test_clean_non_twitter_url(self):
        ...
        self.assertEqual(
            cm.exception.block_errors,
            {"tweet": ValidationError("Please enter a valid Twitter URL.")},
        )
    ...
    def test_clean_http_www_twitter_url(self):
        ...
        with self.assertRaises(ValidationError) as cm:
            block.clean(
                {
                    "tweet": EmbedValue(
                        "http://www.twitter.com/WagtailCMS/status/1413141835711606786"
                    ),
                }
            )
        self.assertEqual(
    ...
    def test_clean_https_www_twitter_url(self):
        ...
        with self.assertRaises(ValidationError) as cm:
            block.clean(
                {
                    "tweet": EmbedValue(
                        "https://www.twitter.com/WagtailCMS/status/1413141835711606786"
                    ),
                }
            )
