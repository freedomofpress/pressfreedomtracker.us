...
def make_html_string():
    ...
    sentence2 = ", ".join([piece.format(word) for piece, word in zip(pieces, words)])
...
def generate_tabs():
    block_fns = [
        functools.partial(generate_heading_field, as_type="heading_2"),
    ...
    page_links = {
        ...
        "table_data": [
            ...
            for i, page in enumerate(
                Page.objects.filter(depth__gt=1).order_by("?")[:num_pages]
            )
        ],
    }
