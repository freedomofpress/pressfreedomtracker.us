...
class FilterFormTest(TestCase):
    def test_filter_type_text(self):
        ...
        name = "search"
        item = {"filters": [{"title": "Search terms", "type": "text", "name": name}]}
    ...
    def test_filter_type_date(self):
        request = RequestFactory().get("/")
        name = "date"
        item = {"filters": [{"title": "Took place", "type": "date", "name": name}]}
        form = FilterForm(request.GET, data=item)
    ...
    def test_filter_type_bool(self):
        ...
        item = {
            "filters": [
                {"title": "Unnecessary use of force?", "type": "bool", "name": name}
            ]
        }
        form = FilterForm(request.GET, data=item)
    ...
    def test_filter_type_checkbox(self):
        ...
        self.assertIsInstance(
            form.fields.get(name).widget, forms.CheckboxSelectMultiple
        )
    ...
    def test_get_filter_forms(self):
        ...
        self.assertIsInstance(
            form[0].fields.get(name).widget, forms.CheckboxSelectMultiple
        )
        self.assertEqual(
