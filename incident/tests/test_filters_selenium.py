...
class FilterTest(SeleniumTest):
    ...
    def open_filters(self):
        self.browser.find_element_by_css_selector(
            ".filters__button--summary-toggle"
        ).click()
...
class BooleanFilterTestCase(FilterTest):
    def setUp(self):
        ...
        self.true_bool = IncidentPageFactory(
            categories=[self.category], charged_under_espionage_act=True
        )
        self.false_bool = IncidentPageFactory(
            categories=[self.category], charged_under_espionage_act=False
        )
    ...
    def test_filters_boolean_true(self):
        ...
        self.open_filters()
        selected_button = self.browser.find_element_by_css_selector(
            "button.radio-pill__item--selected"
        )
...
class MultiRelationFilterTest(FilterTest):
    def setUp(self):
        ...
        self.open_filters()
        self.browser.find_element_by_css_selector(".filters__category").click()
        self.autocomplete_input = self.browser.find_element_by_css_selector(
    ...
    def test_filters_multiple_related_selections(self):
        ...
        for equipment in self.equipment[:2]:
            ...
            self.browser.find_element_by_xpath(
                f'//span[normalize-space(text()) = "{equipment.name}"]'
            ).click()
            wait.until(
                expected_conditions.presence_of_element_located(
                    (
                        By.XPATH,
                        f'//span[text()="{equipment.name}"][@class="selection__label"]',
                    )
                )
            )
        self.apply_filters()
        ...
        self.assertEqual(
            {
                element.text
                for element in self.browser.find_elements_by_css_selector(
                    ".selection__label"
                )
            },
...
class DateFilterTest(FilterTest):
    def setUp(self):
        ...
        self.open_filters()
        self.lower, self.upper = self.browser.find_elements_by_css_selector(
            "input.filter-date-picker"
        )
    ...
    def test_filters_lower_date(self):
        ...
        self.open_filters()
        lower, upper = self.browser.find_elements_by_css_selector(
            "input.filter-date-picker"
        )
...
class ChoiceFilterTest(FilterTest):
    ...
    def test_filters_by_dropdown_choice(self):
        ...
        select = Select(
            self.browser.find_element_by_css_selector("select.choice-input")
        )
