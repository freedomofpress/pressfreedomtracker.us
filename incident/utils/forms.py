            ...
            for item in filters:
                ...
                if _type == "autocomplete":
                    ...
                    for choice in model.objects.all():
                        title_field = getattr(
                            model, "autocomplete_search_field", "title"
                        )
                ...
                if _type == "date":
                    ...
                    kwargs["label"] = f"{label} after"
                    self.fields[f"{name}_lower"] = field(**kwargs)

                    kwargs["label"] = f"{label} before"
                ...
                if _type == "checkbox":
                    ...
                    kwargs["choices"] = [
                        [value, capfirst(display)]
                        for value, display in item.get("choices", [])
                    ]
                ...
                if _type == "radio":
                    ...
                    kwargs["widget"] = forms.RadioSelect
                    kwargs["choices"] = [
                        [x[0], capfirst(x[1])] for x in choices.MAYBE_BOOLEAN
                    ]
...
def get_filter_forms(request, serialized_filters):
    ...
    categories = [item for item in serialized_filters if item.get("id", -1) != -1]
            ...
            "filters": [
                {
                    ...
                    "name": "categories",
                    "choices": [
                        [x.get("id", ""), x.get("title", "")] for x in categories
                    ],
                }
            ],
        }
        filter_forms.insert(0, FilterForm(request.GET, data=item))
