...
class ReplyToFieldsValidatorFormTests(TestCase, WagtailTestUtils):
    ...
    def test_does_not_allow_multiple_fields_with_the_same_label_across_groups(self):
        ...
        response = self.client.post(
            reverse(
                "wagtailadmin_pages:add", args=("forms", "formpage", self.home_page.pk)
            ),
            post_data,
        )
        self.assertEqual(response.status_code, 200)
        ...
        self.assertContains(
            response, "There is another field with the label Test label"
        )
...
class AppendSubjectFieldsTestCase(TestCase):
    ...
    def test_subject_form_fields(self):
        ...
        request = factory.post(
            self.form_page.get_url(), {"topic": topic, "theme": theme}
        )
        form = self.form_page.get_form(
            request.POST,
            request.FILES,
        ...
        self.form_page.send_mail(form)
        self.assertEqual(
            mail.outbox[0].subject, f"{self.form_page.subject} - {topic} - {theme}"
        )
