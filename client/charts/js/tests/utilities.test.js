...
describe(filterDatasetByTag, () => {
	test("filterDatasetByTag simple test", () => {
		expect(
			filterDatasetByTag(
				[{ tags: "test1" }, { tags: "test2" }, { tags: "test1" }],
				"test1",
			),
	...
	test("scopes a rolling-window preset link (e.g. state map) to the whole window", () => {
		const url = getFilteredUrl(
			"/",
			{
				tag: null,
				year: null,
				timePreset: TIME_PRESETS.FOUR_WEEKS,
				state: "New York",
			},
			currentDate,
			[],
...
describe(resolveDefaultTimePreset, () => {
	...
	test("returns SEVEN_DAYS when enabled and there is data in the last 7 days", () => {
		expect(resolveDefaultTimePreset([withinSevenDays], currentDate, true)).toBe(
...
describe(groupByState, () => {
	test("groupByState simple test", () => {
		expect(
			groupByState([{ state: "AK" }, { state: "AL" }, { state: "AK" }]),
		).toEqual([
			{
				state: "Alaska (AK)",
				usCode: "AK",
	...
	test("groupByState unordered dataset", () => {
		expect(
			groupByState([{ state: "AL" }, { state: "AK" }, { state: "AK" }]),
		).toEqual([
			{
				state: "Alaska (AK)",
				usCode: "AK",
...
describe(countIncidentsOutsideUS, () => {
	...
	test("countIncidentsOutsideUS undefined state", () => {
		expect(
			countIncidentsOutsideUS([{ state: "NY" }, { state: undefined }]),
		).toEqual(1);
	});

	test("countIncidentsOutsideUS unknown state", () => {
		expect(
			countIncidentsOutsideUS([{ state: "NY" }, { state: "Russia" }]),
		).toEqual(1);
	});
