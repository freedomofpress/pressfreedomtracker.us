...
export function getFilteredUrl(
	...
	const categoriesSlugs = categories.reduce(
	...
	const baseUrl =
		...
		categoriesSlugs[filtersApplied.category] === undefined
			? `${origin}${databasePath}?`
	...
	if (
		filtersApplied.weekStart !== undefined &&
		filtersApplied.weekEnd !== undefined
	) {
		parameters.push(
			`date_lower=${filtersApplied.weekStart}&date_upper=${filtersApplied.weekEnd}`,
		);
	}

	if (
		filtersApplied.timePreset === TIME_PRESETS.YEAR &&
		filtersApplied.monthName === undefined
	) {
		parameters.push(
			`date_lower=${filtersApplied.year}-01-01&date_upper=${filtersApplied.year}-12-31`,
		);
	}
	...
	if (
		...
		parameters.push(
			`date_lower=${firstDateFormatted}&date_upper=${lastDateFormatted}`,
		);
	}
	...
	if (rollingSpan && filtersApplied.weekStart === undefined) {
		...
		const windowStart = interval.offset(
			interval.floor(currentDate),
			-(count - 1),
		);
		parameters.push(
			`date_lower=${iso(windowStart)}&date_upper=${iso(currentDate)}`,
		);
	}
...
export function filterDatasetByLastSixMonths(dataset, currentDate) {
	...
	return dataset.filter(
		(d) => +d.date > +sixMonthsAgo && +d.date <= +currentDate,
	);
}

// Rolling window ending with a partial period that contains currentDate.
function filterDatasetByLastNPeriods(
	dataset,
	currentDate,
	interval,
	numberOfPeriods,
	...
	const firstPeriodStart = interval.offset(
		currentPeriodStart,
		-(numberOfPeriods - 1),
	);
	return dataset.filter(
		(d) => +d.date >= +firstPeriodStart && +d.date <= +currentDate,
	);
}

export function filterDatasetByLastNDays(dataset, currentDate, numberOfDays) {
	return filterDatasetByLastNPeriods(
		dataset,
		currentDate,
		d3.utcDay,
		numberOfDays,
...
export function resolveDefaultTimePreset(
	dataset,
	currentDate,
	sevenDayEnabled,
) {
	...
	if (
		dataset.length === 0 ||
		filterDatasetByLastNWeeks(dataset, currentDate, 4).length > 0
	) {
...
export function filterDatasetByFiltersApplied(
	originalDataset,
	filtersApplied,
	currentDate,
) {
...
export function filterDatasets(
	...
	const filterCategoryMap = (
		Array.isArray(filterCategories) ? filterCategories : [filterCategories]
	...
	return dataset
		.filter(({ categories, tags, date, state }) => {
			const incidentCategories = categories
				? categories.split(",").map((d) => d.trim())
				: [];
			const incidentTags = tags ? tags.split(",").map((d) => d.trim()) : [];

			const isExcludedCategory =
				filterCategories.length &&
				!incidentCategories.find((c) => filterCategoryMap[c]);
			const isExcludedTag =
				filterTags && !incidentTags.find((c) => filterTagsMap[c]);
...
export function groupByMonthSorted(dataset, isLastSixMonths, currentDate) {
	const datasetGroupedByMonth = d3
		...
		.map((d) => ({
			month: d[0],
			monthName: monthNames[d[0]],
			numberOfIncidents: d[1].length,
		}));
...
export function groupByYearsSorted(dataset) {
	...
	const allYearsGrouped = d3
		.range(yearsExtent[0], yearsExtent[1] + 1)
		.map((year) => [year, yearsGroupedMapped[year] || []]);

	const datasetGroupedByYear = allYearsGrouped.map((d) => ({
		year: d[0],
		numberOfIncidents: d[1].length,
	}));
...
export function groupByDaysSorted(dataset, currentDate, numberOfDays) {
	...
	const days = d3
		.range(numberOfDays)
		.map((i) => d3.utcDay.offset(currentDayStart, -(numberOfDays - 1 - i)));
...
export function groupByState(dataset) {
	...
	const states = dataset.map((d) => ({
		acronym:
			d.state !== undefined && usStatesList.includes(d.state)
				? d.state
				: "Abroad",
	}));
	...
	const incidentsGroupedByState = d3
		...
		.map(([acronym, numberOfIncidents]) => ({
			state:
				acronym !== "Abroad"
					? usStates.find((d) => d.acronym === acronym).state
					: "Abroad",
			usCode: acronym !== "Abroad" ? acronym : null,
			numberOfIncidents,
			latitude:
				acronym !== "Abroad"
					? usStates.find((d) => d.acronym === acronym).latitude
					: undefined,
			longitude:
				acronym !== "Abroad"
					? usStates.find((d) => d.acronym === acronym).longitude
					: undefined,
		}))
		.sort((a, b) => b.numberOfIncidents - a.numberOfIncidents);
...
export function countIncidentsOutsideUS(dataset) {
	...
	return dataset.filter(
		(d) =>
			d.state === undefined ||
			(!usStatesList.includes(d.state) && d.state !== "PR"),
	).length;
}
