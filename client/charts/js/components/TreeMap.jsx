...
import CategoryButtons, {
	calculateCategoriesLabelsLegend,
	calculateButtonsHeight,
} from "./CategoryButtons.jsx";
...
export function computeMinimumNumberOfIncidents(
	dataset,
	chartHeight,
	minimumBarHeight,
) {
...
export function stackDatasetByCategory(
	...
	const categoriesSimplified = [].concat.apply(
		...
		categories.map((d) =>
			d
				.split(categoryDivider)
				.map((e) => e.trim())
				.filter(
					(f) =>
						!filterElements ||
						filterElements.length === 0 ||
						filterElements.includes(f),
				),
		),
	);
	...
	const incidentsGroupedByCategory =
		allCategories !== undefined
			? Object.fromEntries(
					allCategories.map((d) => [
						d,
						categoriesSimplified.filter((e) => e === d).length,
					]),
				)
			: Object.fromEntries(
					d3.rollup(
	...
	const incidentsGroupedByCategoryAdjusted = Object.fromEntries(
		d3.rollup(
			categoriesSimplified.map((d) => ({ category: d })),
			(v) =>
				v.length === 0 ? 0 : Math.max(v.length, minimumNumberOfIncidents),
			(d) => d.category,
		),
	);
	...
	const stack = d3
		.stack()
		.keys(
			Object.keys(incidentsGroupedByCategory).sort((a, b) =>
				a.localeCompare(b),
			),
		);
	return (
		stack([incidentsGroupedByCategoryAdjusted])
			.map((d) => ({
				startingPoint: d[0][0],
			...
			.filter(
				(d) =>
					...
					(filterElements.length && allCategories.indexOf(d.category) >= 0),
			)
	);
}
...
export default function TreeMap({
	...
	const categoryButtonsLabels = calculateCategoriesLabelsLegend(
		datasetStackedByCategory,
		paddings,
		width,
	);
	const buttonsHeight = isMobile
		? 0
	...
	const lengthScale = d3
		...
		.domain(
			dataset.length
				? [0, d3.max(datasetStackedByCategory, (d) => d.endPoint)]
				: [-1, 0],
		)
						...
						content={
							...
							>
								...
								>
									<div
										style={{
											borderLeft: `solid 3px ${findColor(hoveredElement)}`,
											paddingLeft: 3,
										}}
									>
										{hoveredElement}
									...
									<div>
										{
											datasetStackedByCategory.find(
												(d) => d.category === hoveredElement,
											).numberOfIncidents
										}
									</div>
								</div>
			...
			<>
				...
				>
					...
					>
						<Dataset
							...
							init={{
								...
								[chartLengthDimension]: isMobile
									? chartLength - chartLengthPaddingAfter
									: 0,
								[chartWidthTitle]:
									chartWidth -
									(chartWidthPaddingBefore + chartWidthPaddingAfter),
								[chartLengthTitle]: 0,
							}}
							attrs={{
								opacity: 1,
								[chartWidthDimension]: chartWidthPaddingBefore + buttonsHeight,
								[chartLengthDimension]: (d) =>
									chartLength - lengthScale(d.startingPoint),
								[chartWidthTitle]:
									chartWidth -
									(chartWidthPaddingBefore +
										buttonsHeight +
										chartWidthPaddingAfter),
								[chartLengthTitle]: (d) =>
								...
								stroke: (d) =>
									hoveredElement === d.category
										? findColor(d.category)
										: "black",
								strokeWidth: isHomePageDesktopView
									? borderWidth.normal
									: borderWidth.mobile,
								cursor: interactive && searchPageURL ? "pointer" : "inherit",
								pointerEvents: (d) =>
									d.numberOfIncidents === 0 ? "none" : null,
								shapeRendering: "crispEdges",
							}}
							events={{
					...
					<Dataset
						...
						init={{
							opacity: 0,
							[`${chartWidthDimension}1`]:
								chartWidthPaddingBefore +
								buttonsHeight -
								borderWidth.normal / 2,
							[`${chartWidthDimension}2`]:
								chartWidth - chartWidthPaddingAfter + borderWidth.normal / 2,
							[`${chartLengthDimension}1`]: isMobile
								? chartLength - chartLengthPaddingAfter
								: 0,
							[`${chartLengthDimension}2`]: isMobile
								? chartLength - chartLengthPaddingAfter
								: 0,
						}}
						attrs={{
							...
							stroke: (d, i) =>
								hoveredElement === d.category
									...
									: hoveredElement !== null &&
										  hoveredElement ===
												nextCategory(datasetStackedByCategory, i)
