	...
	switch (type) {
		...
		case SET_PARAMETER:
			if (!Object.hasOwn(state, payload.filterName)) {
				throw new Error(
					`Cannot set parameter for unknown filter name "${payload.filterName}"`,
				);
			}
...
export default function FiltersIntegration({
	width,
				...
				case "filterTimeMonths":
					return (d) =>
						new Date(d.date).getTime() >=
							filtersParameters[filterName].parameters.min.getTime() &&
						new Date(d.date).getTime() <=
							filtersParameters[filterName].parameters.max.getTime();
				case "filterTimeYears":
					return (d) =>
						filtersParameters[filterName].parameters.includes(
	...
	function makeUrlParams() {
		...
		filterDefs.forEach((category) => {
			if (
				category.id === -1 ||
				filtersParameters.categories.parameters.has(category.title)
			) {
				if (category.id !== -1) {
				...
				category.filters.forEach(({ name }) => {
					...
					if (filter && filter.parameters) {
						if (name == "tags" && filter.parameters?.size > 0) {
							searchParams.append(
								"tags",
								Array.from(filter.parameters).join(","),
							);
						} else if (
							...
							if (filter.parameters.min) {
								searchParams.append(
									`${name}_lower`,
									filter.parameters.min?.toISOString()?.substring(0, 10),
								);
							}
							if (filter.parameters.max) {
								searchParams.append(
									`${name}_upper`,
									filter.parameters.max?.toISOString()?.substring(0, 10),
								);
							}
						}
					}
		...
		<FiltersDispatch.Provider value={updateFilters}>
			...
			>
				<div className="filters__form--header">
					<h2 className="filter__heading" id="filter-sidebar-heading">
						Filters
					</h2>
					<a className="btn btn-ghost" href="/all-incidents/">
						Clear All
					</a>
				</div>
				...
				<div className="incident-index__filters-mobile-controls">
					<a
						className="incident-index__filters-mobile-clear"
						href="{{ page.url }}"
					>
						Clear All
					</a>
