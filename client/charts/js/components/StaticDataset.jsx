 ...
 * @constructor
 */
export default function StaticDataset({
	dataset,
	tag,
	attrs,
	keyFn,
		...
		React.createElement(
			tag,
			{
				...
				...attrsList.reduce(
					(acc, val) => ({
						...acc,
						[val]:
							typeof attrs[val] === "function" ? attrs[val](data) : attrs[val],
					}),
					{},
				),
			},
			...
			(children &&
				React.Children.toArray(children).map((child) =>
					React.cloneElement(child, { key: child.toString(), dataset: [data] }),
				)) ||
				// Or if the child should be text
				(attrs.text && typeof attrs.text === "function"
					? attrs.text(data)
					: attrs.text),
		),
	);
