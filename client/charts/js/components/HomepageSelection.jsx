...
export default function HomepageSelection({
	...
	function updateSelectedYear(label) {
		...
		const presetKey = Object.keys(TIME_LABELS).find(
			(key) => TIME_LABELS[key] === label,
		);
		setFiltersApplied({
			...filtersApplied,
			year: null,
	...
	function isTimeButtonSelectable(label) {
		switch (label) {
			case TIME_LABELS.SEVEN_DAYS:
				return presetHasData((d) =>
					filterDatasetByLastNDays(d, currentDate, 7),
				);
			case TIME_LABELS.FOUR_WEEKS:
				return presetHasData((d) =>
					filterDatasetByLastNWeeks(d, currentDate, 4),
				);
			case TIME_LABELS.TWELVE_WEEKS:
				return presetHasData((d) =>
					filterDatasetByLastNWeeks(d, currentDate, 12),
				);
			case TIME_LABELS.SIX_MONTHS:
				return presetHasData((d) =>
					filterDatasetByLastSixMonths(d, currentDate),
				);
			case TIME_LABELS.ALL_TIME:
				return true;
			default:
				return presetHasData((d) => filterDatasetByYear(d, label));
		}
	}
		...
		<div>
			<ButtonsRow
				...
				isButtonSelectable={(tag) => {
					return (
						tag === "All incidents" ||
						isTagSelectable(originalDataset, tag, currentDate)
					);
				}}
				tooltipIfUnselectable={
					"No incidents with this tag in specified date range"
				}
			/>
			<ButtonsRow
				label="from"
