...
export function calculateCategoriesLabelsLegend(
...
export function calculateButtonsHeight(categoryButtonsLabels) {
	return (
		d3.max(categoryButtonsLabels, (d) => d.labelStartingY) + labelHeight * 1.5
	);
}
...
export default function CategoryButtons({
	...
	const fillColor = (d) => {
		...
		if (!hoveredElement && d.numberOfIncidents !== 0)
			return findColor(d.category);
		return "white";
	};
				...
				.map((d) => (
					...
					>
						<rect
							...
							stroke={
								hoveredElement === d.category ? findColor(d.category) : "black"
							}
							fill={fillColor(d)}
						/>
						<text
