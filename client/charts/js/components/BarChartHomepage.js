...
export function BarChartHomepage({
	...
	const barDistance = isLastSixMonths
		? barDistances.semester
		: barDistances.year;
	const chartWidth =
		width -
		paddings.left -
		paddings.right -
		paddingsInternal.left -
		paddingsInternal.right;
	...
	const baseGridValue =
		Math.ceil(
			Math.ceil(
				d3.max(datasetGroupedByMonth, (d) => d.numberOfIncidents) /
					numberOfGridLines
			) / 10
		) * 10;

	const gridLines = d3
		.range(0, numberOfGridLines + 1)
		.map((d) => d * baseGridValue);
	...
	const x = (barIndex) => {
		return (
			(chartWidth * barIndex) / numberOfBars +
			paddings.left +
			paddingsInternal.left +
			barDistance
		);
	...
	const openSearchPage = (month) => {
		...
		const lastDayMonth = `${year}-${monthNumber}-${new Date(
			year,
			monthNumber,
			0
		).getDate()}`;
				...
				<g key={"Bar-" + d.monthName}>
					<rect
						...
						style={{
							fill: "#E07A5F",
							strokeWidth:
								hoveredElement === d.monthName ? borders.hover : borders.normal,
							stroke: "black",
							cursor: "pointer",
						}}
