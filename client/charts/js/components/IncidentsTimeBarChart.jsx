...
export function processIncidentsTimeData(
	filteredDataset,
	timePeriod,
	branchFieldName,
	groupByTag,
	...
	const genIncidentsByTime = (dateFn) =>
		filteredDataset.reduce((acc, d) => {
			...
			if (groupByTag) {
				...
				if (d.tags?.indexOf(groupByTag) >= 0) {
					dateData[groupByTag] = dateData[groupByTag]
						? dateData[groupByTag] + 1
						: 1;
				} else {
					const notBranchFieldName = `not ${groupByTag}`;
					dateData[notBranchFieldName] = dateData[notBranchFieldName]
						? dateData[notBranchFieldName] + 1
						: 1;
				}
				...
				branchValues.forEach((branchName) => {
					dateData[branchName] = dateData[branchName]
						? dateData[branchName] + 1
						: 1;
				});
			}
	...
	const showByYears = timePeriod
		? timePeriod === "years"
		: Object.keys(incidentsByMonth).length > 24;
	...
	const allTime = [
		...timeIntervalFn(
			...d3.extent(
				filteredDataset.map(({ date, ...restProps }) => ({
					...restProps,
					date: showByYears ? d3.utcYear.floor(date) : date,
				})),
				(d) => d.date,
			),
		),
		...
		d3.max(
			Object.keys(incidentsByTime).map(
				d3.utcParse(showByYears ? "%Y" : "%Y-%m"),
			),
		),
	].filter((d) => d);
	...
	const xFormat = (date) => {
		...
		if (Object.keys(incidentsByYear).length === 1)
			return d3.utcFormat("%b")(date);
		// If this is the first item in the list, format with the year
		if (
			d3.utcFormat("%m-%Y")(date) === d3.utcFormat("%m-%Y")(allTime[0]) ||
			d3.utcFormat("%m")(date) === "01"
		)
	...
	const incidentsByAllTime = allTime
		.map((date) => ({
			date,
			...(incidentsByTime[timeFormat(date)] || { count: 0 }),
		}))
...
export default function IncidentsTimeBarChart({
	...
	const tagBranches = groupByTag && [
		{ title: groupByTag },
		{ title: `not ${groupByTag}` },
	];

	const { incidentsByAllTime, xFormat, showByYears, allTime } =
		processIncidentsTimeData(
			filteredDataset,
			timePeriod,
			branchFieldName,
			groupByTag,
	...
	const categoriesColorMap =
		...
		[...new Set([...(tagBranches || branches).map((d) => d.title)])].reduce(
			(acc, category, i) => ({
				...acc,
	...
	return (
		<ParentSize>
			{(parent) => {
				const isMobile = parent.width
					? parent.width < tabletMinMainColumn
					: isMobileView;
				const barchart = fullSize ? (
					<BarChart
						...
						allCategories={
							categoriesColorMap && Object.keys(categoriesColorMap)
						}
						x={"date"}
						y={"count"}
						xFormat={xFormat}
						...
						width={parent.width}
						height={
							Math.min(parent.width * (isMobile ? 1 : 0.75), 600) *
							(branchFieldName ? 1.2 : 1)
						}
...
IncidentsTimeBarChart.propTypes = {
	...
	branchFieldName: PropTypes.string,
	branches: PropTypes.array,
	groupByTag: PropTypes.string,
};
