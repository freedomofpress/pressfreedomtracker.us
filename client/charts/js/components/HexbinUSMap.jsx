...
export default function HexbinUSMap({
	...
	const colorScaleInterpolator = d3
		.scaleLinear()
		.domain(COLOR_BREAKS)
	...
	const colorScale = d3
		.scaleSequentialPow(colorScaleInterpolator)
		.exponent(COLOR_SCALE_EXPONENT)
	...
	const gridHeight =
		(maxY - minY + 1) * hexHeight * hexVerticalSpacing +
		hexHeight * hexTopPadding;

	const availableWidth =
		width - margins.left - margins.right - paddings.left - paddings.right;
	const availableHeight =
		height - paddings.bottom - paddings.top - paddings.map;
	...
	const offsetX =
		(availableWidth - gridWidth * scale) / 2 + margins.left + paddings.left;
	const offsetY =
		(availableHeight - gridHeight * scale) / 2 + margins.top + paddings.top;
		...
		<>
			{hoveredElement &&
				...
				(() => {
					const dataPoint = dataset.find(
						(d) => `${aggregationLocality(d)}` === hoveredElement,
					);
							...
							content={
								...
								>
									...
									>
										<div
											style={{
												borderLeft: `solid 3px ${tooltipColor}`,
												paddingLeft: 3,
											}}
										>
											{hoveredElement}
			...
			>
				...
				<g role="list" aria-label="US states with incident data">
					{hexbinCoordinates
						.sort((a, b) => {
							...
							const getIncidents = (hexState) =>
								(
									dataLookup.get(normalizeStateName(hexState.state)) ||
									dataLookup.get(hexState.acronym.toLowerCase())
								)?.numberOfIncidents || 0;
						...
						.map((hexState) => {
							...
							const dataPoint =
								dataLookup.get(normalizedName) || dataLookup.get(stateAcronym);
							...
							const hexX =
								offsetX +
								col * hexWidth * scale +
								(row % 2 === 1 ? hexWidth * 0.5 * scale : 0);
							const hexY =
								offsetY + row * hexHeight * hexVerticalSpacing * scale;

							const incidents = dataPoint
								? dataPoint.numberOfIncidents || 0
								: 0;
							const fillColor = incidents > 0 ? colorScale(incidents) : "white";
							const stateName = dataPoint
								? `${aggregationLocality(dataPoint)}`
								: hexState.state;
							...
							const textColor =
								incidents > 0
									? incidents > maxIncidents * CONTRAST_THRESHOLD
										? "#fff"
										: "#000"
									: "#767676";
								...
								<g key={hexState.acronym}>
										...
										wrapperComponent={
											<a
												href={
													searchPageURL &&
													dataPoint &&
													searchPageURL(dataPoint.usCode)
												}
									...
									>
										<g
											style={{
												cursor:
													interactive && searchPageURL && dataPoint
														? "pointer"
														: "inherit",
											}}
				...
				{maxIncidents > 0 && fullSize && (
					<g
						transform={`translate(${width - 120}, ${height - 120})`}
						role="img"
						aria-label="Legend: Color scale"
					>
						<text
							x={0}
							y={0}
						...
						{(() => {
							...
							const oneThirdPosition =
								Math.pow(oneThirdValue / maxIncidents, COLOR_SCALE_EXPONENT) *
								100;
							const twoThirdsPosition =
								Math.pow(twoThirdsValue / maxIncidents, COLOR_SCALE_EXPONENT) *
								100;
					...
					<g>
						{interactive ? (
							...
							<rect
								...
								width={width}
								height={
									paddings.text * 2 + hexBorder.frame + (width > 400 ? 14 : 12)
								}
								fill="white"
							/>
						)}
						...
						<AnimatedDataset
							...
							attrs={{
								x:
									width -
									paddings.arrowSmall -
									(hoveredElement === "Abroad" ? paddings.arrow : 95),
								y:
									height -
									paddings.bottom -
									paddings.text -
