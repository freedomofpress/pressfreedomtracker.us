...
export default function Search({ data = [], selectedTags = [] }) {
	...
	const categories = Object.keys(
		data.reduce((categoryMap, incident) => {
			incident.categories
				.split(",")
	...
	const frequentTags =
		selectedTags && selectedTags.length
			? selectedTags
			: chooseTrendingTags(data, numberOfTags);
	...
	const updateSearchText = (text) => {
		...
		setSearchTextCateogories(
			categories.filter(
				(category) => category.toLowerCase().indexOf(text.toLowerCase()) >= 0,
			),
		);
	};
	...
	const handleArrowKeys = (event) => {
		...
		const allDropdowns = [
			...document.querySelectorAll(
				".search-dropdown--tag, .search-dropdown--category",
			),
		];
		let currentTagIndex;
		...
		switch (event.keyCode) {
			case 38: // up
				...
				if (dropdownsReversed[currentTagIndex + 1])
					dropdownsReversed[currentTagIndex + 1]
			...
			case 40: // down
				// select next tag
				currentTagIndex =
					allDropdowns.findIndex((tagEl) => tagEl.id === currentId) || 0;
				if (allDropdowns[currentTagIndex + 1])
					allDropdowns[currentTagIndex + 1].querySelector("button").focus();
				event.preventDefault();
				break;
			default:
		}
	...
	return (
		<form
			className={classNames("search-form", {
				"smart-search--unselectable": unselectable,
			})}
		...
		>
			...
			<input
				...
				ref={inputRef}
				placeholder={
					!selectedTag && !selectedCategory ? "Search incidents" : ""
				}
				spellCheck="false"
				autoComplete="off"
				type="search"
				className={classNames(
					...
					searchActive && "smart-search-active",
				)}
				name="search"
				value={searchText}
			...
			{selectedCategory && (
				<div className="search-category-pill">
					<span className="search-category-pill--category-label">
						category:{" "}
					</span>
			...
			{searchActive &&
				...
				!!frequentTags.length && (
					<div className="search-dropdown" id="search-dropdown" role="listbox">
						{searchText ? (
							<>
								<div className="search-dropdown--header">Search term</div>
											...
											{searchTextCategories.map((category, i) => (
												...
												>
													<button
														...
														onKeyDown={handleArrowKeys}
													>
														<span className="search-dropdown--tag--button--category-label">
															category:
														</span>
														<CategoryIcon category={category} />
														{category}
														<span className="search-dropdown__go">
															Go &#8594;
														</span>
													</button>
												</li>
									...
									{frequentTags.map((tag, i) => (
										...
										>
											<button
												...
												onKeyDown={handleArrowKeys}
											>
												<span className="search-dropdown--tag--button--hash">
													#
												</span>
												{tag}
