...
function PrepubBarChart(props) {
	return (
		<ParentSize>
			{(parent) => (
				<PrepubBarChartWidth {...props} parentWidth={parent.width} />
			)}
		</ParentSize>
	);
}

function PrepubBarChartWidth({ width, dataset, parentWidth }) {
				...
				<Tooltip
					...
					content={
						<div className="category-count-tooltip">
							<p className="category-count-tooltip--heading">
								Categories tracked (unconfirmed)
							</p>
							{categoryCounts.map((item) => (
								<dl key={item.category} className="category-count-tooltip--row">
									...
									<dd className="category-count-tooltip--count">
										{item.count}
									</dd>
								</dl>
							))}
...
chartContainers.forEach((node) => {
	...
	root.render(<PrepubBarChart width={670} dataset={chartDataset} />);
});

const incidentCountContainers = Array.from(
	document.getElementsByClassName("js-unconfirmed-incidents-count"),
);
